"""Collect D080 source/ELF evidence from board Linux without attaching to the MCU."""
import json
import os
...
folder = os.environ['SUMO_REMOTE_ROOT'] + '/' + source + '/p2_imu_setup_compile'
program = r'''
...
for path in sorted((root/'artifacts'/mode).glob('*.elf')):
    ...
    markers=('imu::','imu_setup_probe::','<setup>','<loop>',' setup',' loop',
...
path = repo / 'state/analysis/P2_imu_setup_raw' / ('target_' + source[:8] + '_' + mode + '.json')
