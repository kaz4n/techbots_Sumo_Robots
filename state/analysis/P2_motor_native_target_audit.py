"""Collect D077 source/ELF evidence from board Linux without attaching to the MCU."""
import json
import os
...
folder = os.environ['SUMO_REMOTE_ROOT'] + '/' + source + '/p2_motor_native_compile'
program = r'''
...
for path in sorted((root/'artifacts'/mode).glob('*.elf')):
    ...
    markers=('motors::','native_motor_probe::','<setup>','<loop>',' setup',' loop',
...
path = repo / 'state/analysis/P2_motor_native_raw' / ('target_' + source[:8] + '_' + mode + '.json')
