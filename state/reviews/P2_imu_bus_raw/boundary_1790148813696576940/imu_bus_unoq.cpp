...
bool environmentValid() {
    return devicesAvailable() && irqsUnused() && suppliesValid() && clocksValid() &&
        ...
        LL_GPIO_IsPinLocked(GPIOD_NS, PINS) == 0U;
}
...
BusTransfer Bus::failed(BusStatus status, const Operation& op) {
    ready_ = false;
    faulted_ = true;
    cleanup_ = disableOwned();
    BusTransfer result{};
...
BusStatus Bus::observe(Operation& op, std::uint32_t allowed, std::uint32_t& flags) {
    ...
    if (!ownershipValid()) return BusStatus::OWNERSHIP;
    flags = I2C4_NS->ISR;
    op.error_flags |= flags & ERRORS;
    auto status = errorStatus(flags);
    if (status != BusStatus::OK) return status;
    if ((flags & EVENTS & ~allowed) != 0U || (flags & (I2C_ISR_DIR | I2C_ISR_ADDCODE)) != 0U)
