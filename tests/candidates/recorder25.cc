// Checks the D-071 isolated 25 Hz experiment against B15 and frozen public contracts.
// Keeps 25 Hz expectations separate from the production CMake test source glob.
// Compile with candidate core and recorder sources; optional unchanged locked tests.
...
TEST_CASE("D071 B15 candidate configuration has literal25Hz5001frame capacity") {
...
TEST_CASE("D071 B15 candidate retains every40ms endpoint0through200000 without overwrite") {
...
TEST_CASE("D071 B15 candidate permits one offcadence final endpoint within capacity") {
    recorder::FrameBuffer frames;
    for (std::uint32_t i = 0U; i < 5000U; ++i)
        CHECK(frames.append(timestamp(i * 40U), logframe::PackStatus::OK));
...
TEST_CASE("D071 B15 real Robot candidate emits phaseanchored40ms frames after receipts") {
    robot_test::Rig rig;
...
TEST_CASE("D071 B15 real Robot candidate skipped40ms slots remain explicit without replay") {
    robot_test::Rig rig;
    const auto anchor = robot_test::release(rig, robot_test::idle(rig));
...
TEST_CASE("D071 B15 candidate forced final frame coalesces at cadence and seals with receipt") {
