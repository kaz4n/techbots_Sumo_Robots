# Exercises the D-064 default-only inert GPIO upload boundary with fake transports.
...
class GpioUploadTests(unittest.TestCase):
    def fixture(self, transport):
        ...
        shutil.copytree(ssh_fixture.PROJECT / 'bench/p0_gpio',
                        fixture.root / 'bench/p0_gpio')
        return fixture
    ...
    def test_default_upload_follows_compile_and_uses_same_inert_artifact(self):
        for transport in ('ssh', 'adb'):
            with self.subTest(transport=transport):
                ...
                result = fixture.run_tool('bench/p0_gpio')
    ...
    def test_compile_only_all_orders_never_uploads_even_with_match(self):
        for transport in ('ssh', 'adb'):
            ...
            for flags in ((), ('--match',), ('--startup', 'immediate')):
                orders = (list(itertools.permutations(('bench/p0_gpio', '--compile-only', *flags)))
                          if '--startup' not in flags else
                          [('bench/p0_gpio', '--compile-only', *flags)])
                for args in orders:
    ...
    def test_unreviewed_immediate_or_motor_upload_rejected_before_transport(self):
        for transport in ('ssh', 'adb'):
            fixture = self.fixture(transport)
            for flags in (('--match',), ('--startup', 'immediate')):
                result = fixture.run_tool('bench/p0_gpio', *flags)
                self.assertNotEqual(0, result.returncode)
                self.assertEqual([], fixture.events)

    def test_changed_gpio_source_rejected_before_transport(self):
        for transport in ('ssh', 'adb'):
            fixture = self.fixture(transport)
            fixture.write('bench/p0_gpio/src/extra.cpp', 'int changed_source;\n')
            result = fixture.run_tool('bench/p0_gpio')
            self.assertNotEqual(0, result.returncode)
            self.assertEqual([], fixture.events)

    def test_gpio_compile_failure_prevents_upload(self):
        for transport in ('ssh', 'adb'):
            fixture = self.fixture(transport)
            result = fixture.run_tool('bench/p0_gpio', changes={'FAKE_FAIL': 'compile'})
            self.assertNotEqual(0, result.returncode)
            self.assertEqual([], fixture.commands('upload'))
